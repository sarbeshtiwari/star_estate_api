const FAQModel = require('../models/projectConfigurationFAQ');

exports.addFaqByPropertyType = async (req, res) => {
...
exports.getFAQByPropertyTypeAndType = async (req, res) => {
    const { propertyType, faqType } = req.params;

    try {
        const faqs = await FAQModel.find({ propertyType, faqType });
        console.log('faqs')
        res.json(faqs);
    } catch (err) {
        console.error(err);
